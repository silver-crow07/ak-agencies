...
function CustomersContent() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-2xl text-[#241B18]">Customers</h1>
        <p className="text-sm font-sans text-[#6B5E57] mt-1">Manage your customers</p>
      </div>
          ...
          <div className="animate-pulse">
            ...
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="h-16 border-b border-[#E8DFD6] last:border-0 flex items-center px-5 gap-4">
                ...
                <div className="h-4 w-40 bg-[#F8F3EA] rounded" />
                <div className="h-4 w-24 bg-[#F8F3EA] rounded ml-auto" />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-[#E8DFD6] overflow-hidden">
          {customers.length === 0 ? (
            <div className="p-12 text-center">
              <Users className="mx-auto mb-3 text-[#6B5E57]/40" size={40} />
              <p className="text-sm font-sans text-[#6B5E57]">No customers found</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm font-sans">
                <thead>
                  <tr className="border-b border-[#E8DFD6] bg-[#F8F3EA]/50">
                    ...
                    <th className="text-left px-5 py-3 font-medium text-[#6B5E57]">Joined</th>
                    <th className="text-center px-5 py-3 font-medium text-[#6B5E57]">Orders</th>
                    <th className="text-right px-5 py-3 font-medium text-[#6B5E57]">Total Spent</th>
                ...
                <tbody>
                  {customers.map((customer) => (
                    ...
                    >
                      <td className="px-5 py-3 font-medium text-[#241B18]">{customer.name}</td>
                      <td className="px-5 py-3 text-[#6B5E57]">{customer.email}</td>
                      <td className="px-5 py-3 text-[#6B5E57]">{customer.phone ?? '—'}</td>
                      ...
                      <td className="px-5 py-3 text-center text-[#6B5E57]">{customer.orderCount}</td>
                      <td className="px-5 py-3 text-right font-medium text-[#241B18]">{formatPrice(customer.totalSpent)}</td>
